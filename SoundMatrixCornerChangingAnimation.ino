...
void loop() {
  ...
  if (!analogRead(A5)) { // If there is low voltage on A5
    voltReading = map(voltReading, 2, 24, 1, 5);//Map the volt reading to a value between 1 and 5
    for (uint8_t j = 0; j < voltReading; j++) {
      digitalWrite(latch, LOW); // Writes the latch pin to low
      shiftOut(data, clk, LSBFIRST, cornersData[j]); // Shifts out the corners animation
    ...
    if (analogRead(A4)) { //If there is a low voltage on A4
      voltReading = map(voltReading, 2, 24, 1, 8);//Map the volt reading to a value between 1 and 5
      for (uint8_t j = 0; j < voltReading; j++) {
      ...
      voltReading = map(voltReading, 2, 24, 1, 4);;//Map the volt reading to a value between 1 and 8
      for (uint8_t j = 0; j < voltReading; j++) {
        digitalWrite(latch, LOW);
        shiftOut(data, clk, LSBFIRST, boxData[j]); // Shifts out the corner animation
