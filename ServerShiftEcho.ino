...
void setup() {
  ...
  pinMode(ds, OUTPUT); //Sets dataPin to OUTPUT
  pinMode(clk, OUTPUT);//Sets clockPin to OUTPUT
  pinMode(l, OUTPUT);//Sets latchPin to OUTPUT
  pinMode(GND, OUTPUT);//Sets clockPin to OUTPUT
  pinMode(VCC, OUTPUT);//Sets latchPin to OUTPUT
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    digitalWrite(l, LOW);
    shiftOut(ds, clk, LSBFIRST, incomingByte);
    digitalWrite(l, HIGH);
  }
}
